...
export default function Home() {
  ...
  return (
    <main className="flex w-full min-h-screen justify-center">
      ...
      <div className="fixed bg-black h-20 w-full   justify-center flex items-center "> <div className="w-72 h-8 gap-x-10 rounded-md"><input
        type="text"
        ...
        <div className="pt-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 ">
          {videos.map((video: any, index: any) => (
            <div className="w-72  h-60 rounded-md my-2 border" key={video.id.videoId + `${index}-${Date.now()}`}>
              <div className="w-72 p-2 h-40  flex items-center justify-center rounded-md"><img className=" p-1  w-full h-full" src={video.snippet.thumbnails.medium.url} alt={video.snippet.title} /></div>
